...
// PAGE NAVIGATION
function showPage(name){document.querySelectorAll('.page').forEach(p=>p.classList.remove('active'));document.getElementById('page-'+name).classList.add('active');document.querySelectorAll('.nav-links a').forEach(a=>a.classList.remove('active'));const navEl=document.getElementById('nav-'+name);if(navEl)navEl.classList.add('active');currentPage=name;window.scrollTo({top:0,behavior:'smooth'});if(name==='shop')renderShop();if(name==='careers')renderJobs();if(name==='home')renderHomeProducts();}
function goToSection(section){const homeVisible=document.getElementById('page-home').classList.contains('active');if(!homeVisible){showPage('home');requestAnimationFrame(()=>{document.getElementById(section)?.scrollIntoView({behavior:'smooth'});});}else{document.getElementById(section)?.scrollIntoView({behavior:'smooth'});}}
...
function addToCartFromModal(){const p=PRODUCTS.find(x=>x.id===currentProdId);if(!p)return;for(let i=0;i<currentQty;i++)cart.push({...p});updateCartBadge();showToast(`${currentQty}× ${p.name} added to cart`);document.getElementById('prodModal').classList.remove('open');}
function addToCartDirect(id){const p=PRODUCTS.find(x=>x.id===id);if(!p||!p.inStock)return;cart.push({...p});updateCartBadge();showToast(`${p.name} added to cart`);}
...
function openCart(){const el=document.getElementById('cartItems');const totalRow=document.getElementById('cartTotalRow');if(!cart.length){el.innerHTML='<div style="text-align:center;padding:32px 0;color:var(--mid)"><div style="font-size:40px;margin-bottom:10px">🛒</div><p style="font-size:14px">Your cart is empty.</p><button class="btn-primary" style="margin-top:12px" onclick="document.getElementById(\'cartModal\').classList.remove(\'open\');showPage(\'shop\')">Start Shopping</button></div>';totalRow.style.display='none';}else{const counts={};cart.forEach(p=>{counts[p.id]=counts[p.id]||{name:p.name,price:p.price,qty:0};counts[p.id].qty++;});el.innerHTML=Object.values(counts).map(c=>`<div class="cart-item-row"><div class="cart-item-name">${c.name}</div><div class="cart-item-right"><span style="font-size:13px;color:var(--mid)">×${c.qty}</span><span class="cart-item-price">₱${(c.price*c.qty).toLocaleString()}</span><button class="cart-item-del" onclick="removeFromCart(${Object.keys(counts).find(k=>counts[k].name===c.name)})">×</button></div></div>`).join('');const total=cart.reduce((sum,p)=>sum+p.price,0);totalRow.style.display='flex';totalRow.innerHTML=`<span>Total</span><span style="color:var(--orange)">₱${total.toLocaleString()}</span>`;}document.getElementById('cartModal').classList.add('open');}
function removeFromCart(id){const idx=cart.findIndex(p=>p.id==id);if(idx!==-1)cart.splice(idx,1);updateCartBadge();openCart();}
...
function checkout(){if(!cart.length){showToast('Your cart is empty!');return;}const total=cart.reduce((s,p)=>s+p.price,0);cart=[];updateCartBadge();document.getElementById('cartModal').classList.remove('open');showToast(`Order of ₱${total.toLocaleString()} placed! We'll contact you shortly.`);}
...
// SIGN IN
function openSignIn(){document.getElementById('signInModal').classList.add('open');}
...
function signIn(){const em=document.getElementById('siEmail').value;const pw=document.getElementById('siPw').value;if(!em||!pw){showToast('Please enter your email and password');return;}document.getElementById('signInModal').classList.remove('open');showToast('Signed in successfully! Welcome back.');}
...
Keep responses friendly, concise, and helpful. Use ₱ for prices. If asked about specific pricing, give ranges from the categories above. Encourage customers to visit a branch or call for exact quotes. Reply in the same language the customer uses (Filipino or English).`;
...
function toggleChat(){
  ...
  if(chatIsOpen){
    ...
    if(!chatHistory.length){
      setTimeout(()=>addBotMessage("Hello! Is there anything I can help you with? 😊\n\nI can assist with product information, branch locations, delivery, and career opportunities at Savers Depot."),400);
    }
...
async function sendChat(){
  ...
  if(!GEMINI_API_KEY||GEMINI_API_KEY==='YOUR_GEMINI_API_KEY_HERE'){
    addBotMessage("⚠️ Please open the HTML file and replace <strong>YOUR_GEMINI_API_KEY_HERE</strong> with your free Gemini API key.<br><br>Get one free at: <a href='https://aistudio.google.com/app/apikey' target='_blank'>aistudio.google.com</a>");
    return;
  }
  ...
  try{
    ...
    hideTyping();
    if(data.error){
      addBotMessage(`Error: ${data.error.message}. Please check your API key.`);
    }else{
      const reply=data.candidates?.[0]?.content?.parts?.[0]?.text||"Sorry, I couldn't process that. Please call us at +63 32 422-6223.";
      chatHistory.push({role:'assistant',content:reply});
      addBotMessage(reply);
    }
  }catch(err){
    hideTyping();
    addBotMessage("Sorry, I'm having trouble connecting right now. Please call us at +63 32 422-6223 or email info@saversdepot.com.ph for assistance.");
  }
